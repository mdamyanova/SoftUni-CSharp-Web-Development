//Write a program that reads a rectangular integer matrix of 
//size N x M and finds in it the square 3 x 3 that has maximal sum of its elements. 
...
class MaximalSum
{
    ...
    static void PrintMatrixWithMaxSum(int[,] matrix)
    {
        ...
        for (int row = 0; row < matrixWithMaxSum.GetLength(0); row++)
        {
            ...
            {
                Console.Write("{0, 4}", matrixWithMaxSum[row, col]);
            }
